(() => {
  ...
  const refs={
    ...
    back:$('#backBtn'),settings:$('#settingsBtn'),nav:$$('.bottom-nav button'),toast:$('#toast'),modal:$('#modal'),modalContent:$('#modalContent'),photoInput:$('#photoInput'),nativeCameraInput:$('#nativeCameraInput'),
    video:$('#cameraVideo'),cameraPlaceholder:$('#cameraPlaceholder'),shutter:$('#shutterBtn'),photoMode:$('#photoModeBtn'),videoMode:$('#videoModeBtn'),recordingStatus:$('#recordingStatus'),recordingTime:$('#recordingTime'),switchCamera:$('#switchCameraBtn'),flash:$('#flashBtn'),gps:$('#gpsBtn'),gpsText:$('#locationText'),mode:$('#cameraMode'),modeBadge:$('#cameraModeBadge'),assistBadge:$('#cameraAssistBadge'),guides:$('#cameraGuides'),levelGuide:$('#levelGuide'),stage:$('#cameraStage'),focus:$('#focusRing'),exposurePanel:$('#exposurePanel'),exposure:$('#exposureSlider'),exposureValue:$('#exposureValue'),zoomPanel:$('#zoomPanel'),zoom:$('#zoomSlider'),zoomValue:$('#zoomValue'),thumb:$('#albumThumb'),thumbEmpty:$('#albumThumbEmpty'),albumThumbBtn:$('#albumThumbBtn'),openImport:$('#openImportBtn'),captureContextBtn:$('#captureContextBtn'),captureContextText:$('#captureContextText'),
    albumGrid:$('#albumGrid'),albumEmpty:$('#albumEmpty'),albumCamera:$('#albumCameraBtn'),albumImport:$('#albumImportBtn'),emptyImport:$('#emptyImportBtn'),selectMode:$('#selectModeBtn'),selectionBar:$('#albumSelectionBar'),selectedCount:$('#selectedCount'),makeReport:$('#makeReportBtn'),deleteSelected:$('#deleteSelectedBtn'),albumSearch:$('#albumSearch'),albumSourceFilter:$('#albumSourceFilter'),
  ...
  async function loadSettings(){
    const saved=await DB.getSetting('settings',state.settings);state.settings={...state.settings,...saved,camera:{...state.settings.camera,...(saved?.camera||{})}};
  ...
  function cameraVideoConstraints(){
    const video={facingMode:{ideal:state.facing},width:{ideal:1920},height:{ideal:1440}},frameRate=preferredFlickerFrameRate();
  ...
  function stopCamera(){state.cameraRequestId+=1;if(state.mediaRecorder?.state==='recording')state.mediaRecorder.stop();if(state.stream)state.stream.getTracks().forEach(t=>t.stop());state.stream=null;state.track=null;refs.video.srcObject=null;state.capabilities={};state.torch=false;refs.flash.querySelector('span').textContent='OFF';refs.exposurePanel.hidden=true;refs.zoomPanel.hidden=true}
  function setupCameraCapabilities(){
    ...
    refs.exposure.value=0;refs.exposureValue.textContent='0.0';refs.exposurePanel.hidden=true;
  ...
  function updateCameraUi(){
    ...
    const active=(camera.grid&&camera.grid!=='off')||camera.center||camera.front||camera.level;refs.assistBadge.hidden=!active;refs.video.style.filter=state.softwareExposure?`brightness(${Math.pow(2,state.softwareExposure/2)})`:'none'
  }
  ...
  async function switchCaptureKind(kind){if(kind===state.captureKind)return;if(state.mediaRecorder){toast('動画の停止処理が終わってから切り替えてください');return}state.captureKind=kind==='video'?'video':'photo';updateCaptureKindUi();stopCamera();await startCamera();toast(state.captureKind==='video'?'動画撮影：停止後に端末へ保存します':'写真撮影に切り替えました')}
  ...
  async function capture(){
    if(!state.stream||!refs.video.videoWidth){openFilePicker(refs.nativeCameraInput);return}
    const c=document.createElement('canvas');c.width=refs.video.videoWidth;c.height=refs.video.videoHeight;const x=c.getContext('2d');x.imageSmoothingEnabled=true;x.imageSmoothingQuality='high';if(state.facing==='user'){x.translate(c.width,0);x.scale(-1,1)}if(state.softwareExposure)x.filter=`brightness(${Math.pow(2,state.softwareExposure/2)})`;x.drawImage(refs.video,0,0,c.width,c.height);x.filter='none';
    ...
    if(state.settings.camera?.phoneSavePrompt){
      let saveFile=null;
      try{saveFile=await makeDeviceSaveFile(photo,state.settings.camera.saveSize||'standard',.94)}catch(error){console.error(error);saveFile=new File([photo.blob],photo.name,{type:photo.blob.type||'image/jpeg'})}
      openCaptureReviewModal(photo,saveFile)
    }else if(!state.settings.continuous)openEditor(photo.id);
  }
  ...
  function renderAlbum(){
    ...
    refs.albumGrid.innerHTML='';refs.albumGrid.classList.toggle('selecting',state.selectionMode);refs.selectionBar.hidden=!state.selectionMode;refs.selectedCount.textContent=state.selected.size;refs.makeReport.disabled=!state.selected.size;
  ...
  async function sharePreparedPhotoFile(file,{afterCapture=false}={}){
    if(navigator.canShare?.({files:[file]})){
      try{await navigator.share({files:[file],title:'WORK PHOTO'});toast('共有画面では「画像を保存」を選択してください');return true}catch(error){if(error?.name==='AbortError'){if(afterCapture)toast('WORK PHOTO内の写真は保存済みです');return false}}
    }
    downloadBlob(file,file.name);toast('共有できないためダウンロードへ保存しました');return false
  }
  async function sharePhotoToDevice(photo,size='original',quality=.94,{afterCapture=false}={}){
    try{
      ...
      return await sharePreparedPhotoFile(file,{afterCapture})
  ...
  function openDeviceSaveModal(photo=currentViewerPhoto()){
    ...
    const prepare=async()=>{const id=++prepareId;preparedFile=null;button.disabled=true;button.textContent='保存データを準備中…';try{const file=await makeDeviceSaveFile(photo,sizeSelect.value,Number(qualitySelect.value));if(id!==prepareId)return;preparedFile=file;button.disabled=false;button.textContent='保存画面へ'}catch(error){console.error(error);if(id!==prepareId)return;button.textContent='準備できませんでした';toast('端末保存用の画像を作成できませんでした')}};
    $('#closeDeviceSave').onclick=close;$('#cancelDeviceSave').onclick=close;sizeSelect.onchange=prepare;qualitySelect.onchange=prepare;
    button.onclick=()=>{if(!preparedFile)return;state.settings.camera.saveSize=sizeSelect.value;DB.setSetting('settings',state.settings).catch(console.error);button.disabled=true;button.textContent='共有画面を開いています…';const sharePromise=sharePreparedPhotoFile(preparedFile);sharePromise.finally(close)};
    prepare()
  }
  ...
  function openCameraSettingsModal(){
    ...
    const camera=state.settings.camera;refs.modalContent.innerHTML=`<div class="compact-modal-head"><div><p class="modal-eyebrow">カメラ</p><h2>撮影設定</h2></div><button id="closeCameraSettings" class="modal-close" aria-label="閉じる">×</button></div><div class="camera-settings-list"><label><span><b>撮影モード</b><small>モードに合うガイドへ切替</small></span><select id="cameraModeSetting">${Object.entries(cameraModeNames).map(([v,l])=>`<option value="${v}"${refs.mode.value===v?' selected':''}>${l}</option>`).join('')}</select></label><label><span><b>グリッド</b><small>構図を合わせる補助線</small></span><select id="cameraGridSetting"><option value="off">表示しない</option><option value="thirds"${camera.grid==='thirds'?' selected':''}>三分割</option></select></label><label><span><b>中央線</b><small>機械の中心を合わせる</small></span><input id="cameraCenterSetting" type="checkbox"${camera.center?' checked':''}></label><label><span><b>正面撮影ガイド</b><small>銘板・書類の位置合わせ</small></span><input id="cameraFrontSetting" type="checkbox"${camera.front?' checked':''}></label><label><span><b>水平器</b><small>端末の傾きを表示</small></span><input id="cameraLevelSetting" type="checkbox"${camera.level?' checked':''}></label><label><span><b>セルフタイマー</b><small>シャッターまでの時間</small></span><select id="cameraTimerSetting"><option value="0">OFF</option><option value="3"${Number(camera.timer)===3?' selected':''}>3秒</option><option value="10"${Number(camera.timer)===10?' selected':''}>10秒</option></select></label><label><span><b>フリッカー軽減</b><small>端末対応時のみ 50Hz→25fps／60Hz→30fps</small></span><select id="cameraFlickerSetting"><option value="off">OFF</option><option value="50"${camera.flicker==='50'?' selected':''}>50Hz地域</option><option value="60"${camera.flicker==='60'?' selected':''}>60Hz地域</option></select></label><label><span><b>撮影後に保存確認</b><small>共有画面で「画像を保存」を選択</small></span><input id="phoneSavePromptSetting" type="checkbox"${camera.phoneSavePrompt!==false?' checked':''}></label><label><span><b>端末保存サイズ</b><small>撮影後・詳細画面で使用</small></span><select id="cameraSaveSizeSetting"><option value="original">元の解像度</option><option value="high"${camera.saveSize==='high'?' selected':''}>高画質 3840px</option><option value="standard"${camera.saveSize==='standard'?' selected':''}>標準 1920px</option><option value="compact"${camera.saveSize==='compact'?' selected':''}>容量優先 1280px</option></select></label></div><div class="camera-quick-actions"><button id="toggleCameraLight" class="soft-btn">常時ライト ${state.torch?'OFF':'ON'}</button><button id="captureGpsNow" class="soft-btn">現在地を取得</button></div><div class="modal-actions two"><button id="openAppSettings" class="soft-btn">アプリ設定</button><button id="applyCameraSettings" class="primary-btn">完了</button></div>`;refs.modal.hidden=false;
    const close=()=>{refs.modal.hidden=true};$('#closeCameraSettings').onclick=close;$('#cameraModeSetting').onchange=()=>{const preview=applyModeAssistDefaults($('#cameraModeSetting').value,{...camera});$('#cameraGridSetting').value=preview.grid;$('#cameraCenterSetting').checked=preview.center;$('#cameraFrontSetting').checked=preview.front};$('#toggleCameraLight').onclick=async()=>{await toggleTorch();$('#toggleCameraLight').textContent=`常時ライト ${state.torch?'OFF':'ON'}`};$('#captureGpsNow').onclick=async()=>{const b=$('#captureGpsNow');b.disabled=true;b.textContent='取得中…';await requestGps({fresh:true});b.disabled=false;b.textContent=state.location?'現在地 取得済み':'現在地を取得'};$('#openAppSettings').onclick=()=>{close();setView('settings')};$('#applyCameraSettings').onclick=async()=>{const oldFlicker=camera.flicker||'off',newFlicker=$('#cameraFlickerSetting').value;refs.mode.value=$('#cameraModeSetting').value;Object.assign(camera,{grid:$('#cameraGridSetting').value,center:$('#cameraCenterSetting').checked,front:$('#cameraFrontSetting').checked,level:$('#cameraLevelSetting').checked,timer:Number($('#cameraTimerSetting').value),flicker:newFlicker,phoneSavePrompt:$('#phoneSavePromptSetting').checked,saveSize:$('#cameraSaveSizeSetting').value});if(state.settings.modeMemory)await DB.setSetting('cameraMode',refs.mode.value);await DB.setSetting('settings',state.settings);if(camera.level){const levelEnabled=await enableLevelGuide();if(!levelEnabled)await DB.setSetting('settings',state.settings)}updateCameraUi();close();if(oldFlicker!==newFlicker){if(newFlicker==='off'){stopCamera();await startCamera();toast('フリッカー軽減をOFFにしました')}else{const applied=await applyFlickerReduction({notify:true});if(!applied){camera.flicker='off';await DB.setSetting('settings',state.settings)}}}else toast('撮影設定を更新しました')}
  }
  function wireEvents(){
    refs.back.onclick=goBack;refs.settings.onclick=()=>state.view==='camera'?openCameraSettingsModal():setView('settings');refs.captureContextBtn.onclick=openCaptureContextModal;refs.editCaptureContext.onclick=openCaptureContextModal;
